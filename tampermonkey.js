...
class BrowserGTE {
    ...
    getMessageSpans = () => {
        ...
        const spanList = Array.from(container.querySelectorAll(this.selectors.spans));
        return spanList
            .slice(spanList.length - this.limit, spanList.length);
    };
        ...
        this.emotes
            .forEach(emote => {
                if (emote.matcher.test(span.innerText)) {
                    ...
                    span.innerHTML = span.innerHTML.replace(emote.matcher, this.api.img(emote.code, emote.id));
                }
            });
    };

    init = async () => {
        this.emotes = await Object.keys(this.channels)
